...
Structural search coverage and economic-evidence completeness remain distinct host facts. If the frozen economic-completeness rule cannot construct a complete deterministic oracle, Agent value may validly be `INCONCLUSIVE` while the independent Candidate Gate still emits `NO_ADAPTIVE_CANDIDATE` when `system_failure = false`. A completed no-candidate campaign is not automatically `SYSTEM_FAILURE` merely because economic evidence is incomplete or an admitted run ends by normal slot exhaustion. Once a matched campaign result is independently accepted, its outcome must not trigger a rerun of that same frozen campaign; any further R4 research uses a new versioned cycle and does not reinterpret the historical result under later executable code.
