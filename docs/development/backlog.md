...
Real 2025 development source/evaluator admission is bound through `R4ResearchAdmission`. The existing `StrictDeepSeekProvider` has an explicit `r4_deepseek_v4_pro` profile and secret-free identity/usage/tariff admission. The original 2026-09-06 non-research probe failed before it retained an accepted identity/usage receipt or failure phase. A later separately authorized non-research probe on **2026-09-07** reached a stronger durable boundary: authenticated endpoint/quota transport succeeded, the response identified `deepseek-v4-pro`, response ID and system fingerprint were present, integer prompt/completion/cache usage was complete, and conservative cost accounting was available in a verified transport receipt. That evidence establishes transport/model/usage reachability for that attempt; it does **not** create provider research authority.

The 2026-09-07 attempt failed in `phase = strict_action`, with no `ProviderAdmission` created. The remaining blocker is exact typed R4 action conformance. Provider admission therefore stays fail-closed: a future separately authorized probe must satisfy the same exact action contract, and there is no provider fallback, automatic retry or weaker “any valid action” rule. The probe implementation now supplies the actual frozen R4 capability manifest, binds that probe contract into `provider_binding()` and retains only allowlisted strict-decoder codes when action parsing fails. No additional real probe is part of this development PR. B-005 remains OPEN, and a new accepted freeze still requires a future separately authorized successful provider admission. The historical [v2 blocked freeze](../../configs/research/r4_matched_v2_blocked/campaign_freeze.json) remains non-executable evidence.

### B-006 — Campaign protocol implemented; admission blocked, comparison pending
...
The guarded real campaign execution operator is now implemented as a thin CLI over the existing `verify_campaign()` and `run_campaign()` application authority. A real `run` requires explicit research admission, provider admission, provider config, campaign directory and exact human-reviewed freeze ID; it exposes no protocol-version, research-rule, retry, force, reset or browser execution authority. Blocked and fixture freezes are not accepted as real execution authority, and an already committed campaign result replays without provider/evaluator calls. This operator changes the executable implementation digest intentionally, so a future accepted v3 freeze must be created only after this code is merged.

The v1/v2 blocked artifacts, including v2 freeze `r4-campaign-freeze-51cdf9a05864e90ffe5310bd`, retain their historical superseded semantics and remain non-executable. Real execution is still blocked by B-005: transport reachability was established on 2026-09-07, but exact strict-action provider admission was not. No corrected accepted freeze, real matched campaign or R4 terminal has occurred. Exposure timing, multi-year inputs, CopilotKit and query migration remain deferred.
