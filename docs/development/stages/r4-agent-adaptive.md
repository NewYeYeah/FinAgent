...
Implemented: causal MarketState/FactorLibrary, five deterministic allocator baselines, session walk-forward evidence, bounded Agent Research Controller, a thin Research Console, and the matched-campaign protocol/runner with the Primary reachability correction. A real matched-budget campaign, accepted AdaptiveStrategy and the R4 terminal remain pending; B-005 provider admission stays open.
...
The engineering boundary is implemented; real execution remains blocked by provider admission. The original v1 and current v2 blocked artifacts under `configs/research/r4_matched_*_blocked/` are immutable history. In particular, the retained v2 artifact has status `BLOCKED_PROVIDER_ADMISSION`, historical protocol ID `r4-matched-protocol-56a7f58549c2bc2d9cba2c78` and freeze ID `r4-campaign-freeze-51cdf9a05864e90ffe5310bd`; it is never accepted by the runner. Its old Agent-value semantics are superseded before any real campaign and are not the current execution protocol. One non-research DeepSeek probe failed verification; no second real probe was made. B-005 remains OPEN. Real 2025 source admission contains no factor/portfolio evaluation results, and no corrected real freeze has been generated.
