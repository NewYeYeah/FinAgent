...
The first completed `r4-matched-v3` cycle ended `NO_ADAPTIVE_CANDIDATE`, so it creates no R5 transition. R5 remains conditional on a future R4 AdaptiveStrategy candidate.
...
Retrospective artifacts bind `evaluation_mode = adaptive_development_retrospective`, `historically_predeclared = false`, `adaptive_search_exposed = true`, `development_only = true` and false independent/Alpha/PAPER/live flags. Internal walk-forward partitions are called **fold evaluation**; the whole Agent search remains exposed development evidence. `DEVELOPMENT_CANDIDATE_PROPOSED` and `NO_CANDIDATE_RECOMMENDED` are Controller recommendations, not the R4 stage terminal. The first frozen matched-budget comparison is complete; independent confirmation remains required only if a future R4 cycle produces a candidate.

## Campaign admission, freeze review, execution and result recording
...
`provider_binding()` includes `probe_contract_digest`, derived from the frozen probe context/action contract. A change to `r4_manifest()`, `PROBE_ACTION` or the probe-context semantics therefore changes provider admission identity. The binding remains secret-free and contains no local path. The transport still has one attempt and no fallback. A failed output directory is immutable evidence and cannot be retried in place.

The historical v1 source admission command and existing admission directories remain history. Do not rebuild or rename the accepted ResearchAdmission merely for publication. The corrected protocol version is code-owned by `src/finagent/research/r4_campaign_protocol.py`: normal accepted freeze generation uses `r4-matched-v3`, while a newly recorded provider-blocked design uses `r4-matched-v3-blocked-provider`. The operator CLI does not expose protocol-version selection; test-only variants such as `r4-matched-v3-test` are fixture-only.

The completed accepted path for this campaign was:

1. separately authorized successful ProviderAdmission;
2. source admission verification;
3. accepted v3 freeze;
4. exact freeze verification;
5. independent freeze review, then STOP;
6. one separately authorized real matched campaign invocation;
7. independent CampaignResult review;
8. canonical repository-safe result recording.

Do not repeat completed admission/freeze/campaign work for an evidence PR. Evidence recording or PR merge is not authorization to execute research. A future R4 research attempt must be a **new versioned cycle** with a new reviewed freeze/result lineage; it may not rerun this accepted matched campaign because of its outcome.

For a separately authorized future versioned admission cycle, freeze/verify/run must use that cycle's own fresh reviewed identities and the existing guarded operator. The original accepted CampaignFreeze and CampaignResult are historical evidence and are not a standing execution authorization.
...
The historical v1/v2 blocked directories and their old `minimum_mean_fold_improvement = 0.002` semantics are immutable evidence. Their labels are superseded and cannot label a newly generated corrected artifact. `campaign_implementation()` binds tracked executable `src/`, `scripts/`, Workspace code and dependency intents, including `scripts/r4_campaign.py`; executable changes after a freeze require a new versioned freeze rather than reinterpretation of the accepted historical result. Full local freeze/result artifacts remain local when their bindings contain private/local paths; repository-safe attestations record exact IDs/hashes and are not executable replacements.

### Accepted first-cycle result

The accepted CampaignResult is `r4-campaign-result-d32ec253d62eb4f9349896b0`, SHA256 `5f9cb2b687f5b5750255c4d91e6db273fdf2577a8ce71f33365cddf19789c8ac`; review bundle SHA256 is `5e387790ec9b6d3198a2439c02900c1817d742de1b123c854f2ca31d4328c69d`. Exactly one campaign invocation completed, with no automatic retry, rerun or provider fallback. Completed runs were deterministic, selection-01, selection-02, selection-03, discovery-01, discovery-02 and discovery-03. Artifact digest audit was 44/44 PASS.

The deterministic host returned `AgentValue = INCONCLUSIVE`, zero successful Agent runs, median portfolio-evaluation saving 3, `deterministic_oracle = null`, `candidate_decision = NO_ADAPTIVE_CANDIDATE` and `candidate_id = null`.

The deterministic search structurally covered all 4 factor sets x 5 allocators = 20 strategies, but 0/20 had complete economic evidence under the frozen three-fold rule. Every deterministic strategy had `evaluable_folds = 0/3`, unavailable sessions ranged from 39 to 64, and all 30 Primary candidate rows were incomplete. Therefore no deterministic economic oracle could be constructed. The accepted no-candidate terminal is **coverage/completeness-driven**, not proof that all strategies lost money or that economic performance was negative; median saving 3 is not positive Agent-value evidence without a complete oracle.

This is not `SYSTEM_FAILURE`: deterministic evaluations completed as `PORTFOLIO_EVALUATED`, all required Primary runs completed, and discovery-03 ended admitted terminal `SLOT_ATTEMPTS_EXHAUSTED`. Candidate completeness and infrastructure failure are separate host gates.

The campaign also retained 62 rejected Agent action attempts. Discovery-03 used 48 provider calls, retained 44 rejected actions and ended `SLOT_ATTEMPTS_EXHAUSTED`; 43 were `candidate_not_proposed_in_run`. ProviderAdmission remains accepted; the observation is a durable autonomous tool-use reliability limitation and the contract is not weakened to conceal it.

No AdaptiveStrategy exists. `development_only = true`, `independent_confirmation = false`, `alpha_authority = false`, `paper_authority = false`, `live_authority = false` and `r5_eligible = false`. R5 therefore does not start. WORKBENCH-2 remains allowed by the current roadmap, while any further R4 research must be a new versioned cycle.
