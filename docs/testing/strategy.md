...
Required only when an R4 AdaptiveStrategy candidate exists:
...
The first completed `r4-matched-v3` cycle produced `candidate_id = null`, so R5 is not started.
...
The accepted real R4 matched campaign is also outside GitHub CI. CI may validate implementation contracts, documentation and repository-safe evidence packaging; it must not call the real provider, access private campaign data/credentials, regenerate the accepted freeze or execute another real matched campaign.
...
Historical v1/v2 blocked freezes remain non-executable regressions and both retain their old `minimum_mean_fold_improvement = 0.002` evidence. Candidate viability thresholds, folds, costs and deterministic ranking remain unchanged. A fixture admission cannot start a real campaign. Provider probes in tests are offline transports; no market objective reaches a real LLM. The original 2026-09-06 non-research probe failed before a verified receipt was retained. An earlier separately authorized 2026-09-07 probe retained complete transport/model/usage evidence but failed exact strict-action admission. That failed evidence remains unchanged. A separately authorized later attempt after hardening passed the exact action and produced accepted ProviderAdmission and an accepted v3 freeze, independently reviewed as `EVIDENCE_ACCEPTED`. B-005 remains resolved. The separately authorized real matched campaign later completed exactly once and independent result review returned `R4_RESULT_ACCEPTED`, resolving B-006. Development CI must never repeat a real probe or campaign. Continue running the existing R3 runtime, static/adaptive walk-forward and Controller regression alongside these tests.

### Accepted offline evidence boundary

The [accepted v3 freeze evidence](../../configs/research/r4_matched_v3_accepted/README.md) records ProviderAdmission `r4-provider-admission-b78a63f33352d5a01bf2a4ca` and freeze `r4-campaign-freeze-1d12fade12cb269d2b4da416`, with two exact-ID verify passes and 34/34 invariant checks. The [accepted campaign result record](../../configs/research/r4_matched_v3_result/README.md) references CampaignResult `r4-campaign-result-d32ec253d62eb4f9349896b0`, SHA256 `5f9cb2b687f5b5750255c4d91e6db273fdf2577a8ce71f33365cddf19789c8ac`, and independent disposition `R4_RESULT_ACCEPTED` without reconstructing the original immutable local result.

Accepted campaign facts are: one real invocation; no automatic retry/rerun/provider fallback; all 7/7 required runs completed; CampaignResult identity PASS; 44/44 artifact digests PASS; provider usage verification PASS; post-run freeze integrity and exact verify PASS; ResearchAdmission/Git environment integrity PASS; `system_failure = false`; host terminal `NO_ADAPTIVE_CANDIDATE`; AgentValue `INCONCLUSIVE`.

The economic-completeness limitation is part of the accepted result, not a CI failure. The deterministic Primary search structurally covered 20/20 factor-set/allocator strategies, but 0/20 had complete required three-fold economic evidence: every deterministic strategy had `evaluable_folds = 0/3`, unavailable sessions ranged from 39 to 64, and all 30 Primary candidate rows were incomplete. Therefore no deterministic economic oracle existed and Agent value remained `INCONCLUSIVE`. This cannot be restated as proof that all strategies lost money or that Agent economic performance was negative.

Discovery-03 ended `SLOT_ATTEMPTS_EXHAUSTED`, an admitted normal campaign-run terminal rather than `SYSTEM_FAILURE`. The campaign retained 62 rejected Agent action attempts overall, including 44 in discovery-03 and 43 `candidate_not_proposed_in_run` rejections there. That reliability limitation is retained in evidence/backlog; tests must not weaken the typed tool contract to improve the statistic.

Evidence-only development validates repository-safe JSON/docs, exact IDs/hashes, authority isolation and governance. It does not probe, regenerate admission/freeze or execute research. Full immutable local campaign artifacts remain outside Git when they carry local/private bindings. Documentation governance and automatically triggered exact-head CI validate packaging without real credentials, market data or a real-provider campaign test.
