...
def _load_toml(path: Path) -> dict[str, object]:
    ...
    values = payload.get("ashare_portfolio_validation")
...
def _fee_schedule(values: Mapping[str, object]) -> AshareFeeSchedule:
    return AshareFeeSchedule(
        ...
        minimum_broker_commission=float(
            values.get("minimum_broker_commission", 5.0)
        ),
        ...
        sse_szse_handling_rate=float(
            values.get("sse_szse_handling_rate", 0.0000341)
        ),
...
def _config(values: Mapping[str, object]) -> AsharePortfolioValidationConfig:
    policy = AsharePortfolioValidationPolicy(
        ...
        max_bootstrap_pvalue=float(
            values.get("policy_max_bootstrap_pvalue", 0.10)
        ),
    ...
    return AsharePortfolioValidationConfig(
        ...
        optimizer_turnover_penalty=float(
            values.get("optimizer_turnover_penalty", 0.01)
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run A4 internal execution-aware A-share portfolio validation from an "
            "immutable A2.6 robust ResearchProgram report. The 2025+ reserve remains "
            "untouched and the output is never promotion-eligible."
        )
    )
    ...
    report_path = args.report or Path(str(values.get("report_path", "reports/ashare_a4.json")))
    ledger_path = args.ledger or Path(str(values.get("ledger_path", "reports/ashare_a4_ledger.jsonl")))
    ...
    codes = tuple(str(value) for value in _sequence(source_universe["ts_codes"], "ts_codes"))
    missing_codes = set(codes) - set(by_code)
    if missing_codes:
        raise ValueError(f"A4 candidate universe is absent from security master: {sorted(missing_codes)}")
    ...
    policy_config = AshareResearchUniversePolicyConfig(
        ...
        min_liquidity_observations=int(
            raw_policy_config["min_liquidity_observations"]
        ),
    ...
    universe_provider, universe_report = AshareResearchUniversePolicy(policy_config).build(
        inference_adapter,
        policy_request,
    ...
    if args.frozen_report is not None:
        ...
        if _mapping(reference["validation_spec"], "validation_spec").get("spec_id") != spec.spec_id:
