...
def _require_new(path: Path) -> Path:
    ...
    if target.exists():
        raise SystemExit(f"US-R2 regime projection evidence is immutable; output exists: {target}")
...
def _source_query(calendar: TradingCalendarEvidence) -> MarketDataQuery:
    ...
    if not sessions:
        raise SystemExit("US-R2 regime projection calendar contains no source sessions")
...
def _local_parquet_plan(plan: USR2RegimeProjectionPlan, parquet_path: Path) -> _LocalParquetPlan:
    return _LocalParquetPlan(
        plan_id=plan.plan_id,
        data_version=plan.data_version,
        sql=f"SELECT * FROM read_parquet({_sql_string(parquet_path.as_posix())}) ORDER BY session_date, fold_id",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize the reusable US-R2 IWM regime/session projection. The expensive source "
            "scan is IWM-only and candidate-independent; the resulting small Parquet is reused by "
            "later 37-candidate materialization."
        )
    )
    ...
    parser.add_argument(
        "--data-output",
        type=Path,
        default=Path("data/us_r2/regime/us_r2_regime_projection.parquet"),
    )
    parser.add_argument(
        "--plan-output",
        type=Path,
        default=Path("reports/us_r2/us_r2_regime_projection_plan.json"),
    )
    parser.add_argument(
        "--evidence-output",
        type=Path,
        default=Path("reports/us_r2/us_r2_regime_projection_evidence.json"),
    )
    ...
    parser.add_argument(
        "--temp-directory",
        type=Path,
        default=Path("data/duckdb_temp/us_r2_regime"),
    )
    return parser
...
def main() -> int:
    ...
    projection_plan = build_us_r2_regime_projection_plan(
        source_plan,
    ...
    if materialization.row_count > MAX_PROJECTION_ROWS:
        raise SystemExit(
            f"US-R2 regime projection exceeded bounded Python review surface: "
            f"{materialization.row_count}>{MAX_PROJECTION_ROWS}"
        )

    # Review/summarize the already materialized tiny projection rather than executing the
    # expensive IWM source plan a second time.
    ...
    evidence = build_us_r2_regime_projection_evidence(projection_plan, materialization, rows)
    ...
    if not evidence.passed:
        raise SystemExit("US-R2 regime projection failed closed; inspect evidence blockers")
    return 0
