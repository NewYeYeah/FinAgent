...
def test_config_registry_redacts_secrets_and_excludes_secret_files(tmp_path: Path) -> None:
    _write(
...
""".strip(),
    ...
    assert "also-must-not-leak" not in str(registry.projection.to_dict())
    assert any("secret-like config excluded without parsing" in item for item in registry.projection.warnings)
...
""".strip(),
    ...
    assert registry.json()["descriptors"][0]["descriptor_id"] == "ashare_portfolio_validation"
    ...
    assert client.post("/api/v3/commands", json={"command_id": "research.run_a2p6"}).status_code == 405
