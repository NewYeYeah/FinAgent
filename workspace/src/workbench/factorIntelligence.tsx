...
function json(value: unknown) { return value == null ? "unavailable" : JSON.stringify(value, null, 2); }
...
export function FactorIntelligencePanel() {
  ...
  const factorId = context.factor_id && indexQuery.data?.items.some((item) => item.factor_id === context.factor_id) ? context.factor_id : undefined;
  const detailQuery = useQuery({ queryKey: marketFactorQueryKeys.factor(factorId ?? ""), queryFn: () => marketFactorApi.factor(factorId ?? ""), enabled: Boolean(factorId), retry: false });
  ...
  if (!factors.length) return <section className="factor-intelligence-shell"><header><div><span className="eyebrow">Workbench-2 · FactorLibrary</span><h2>{t("Factor Intelligence")}</h2></div><span className="experiment-contract">{t("no browser recomputation")}</span></header><div className="market-factor-unavailable">No persisted FactorLibrary is available under the configured report roots. Existing Tear Sheet evidence remains below.</div></section>;

  const detail = detailQuery.data?.item;
  const search = workbenchContextSearch(context);
  return <section className="factor-intelligence-shell" aria-label={t("Factor Intelligence")}>
    <header><div><span className="eyebrow">Workbench-2 · FactorLibrary</span><h2>{t("Factor Intelligence")}</h2><p>{t("Lifecycle, causal state-conditioned development evidence, persisted allocator weights and research lineage.")}</p></div><span className="experiment-contract">{t("GET-only · server authoritative")}</span></header>
    ...
    {detail ? <div className="factor-intelligence-detail">
      <div className="market-factor-grid">
        <article className="factor-intelligence-card"><header><h3>{t("Lifecycle / hypothesis")}</h3><StatusBadge value={detail.status} tone={detail.status === "REJECTED" || detail.status === "RETIRED" ? "negative" : "neutral"} /></header><dl className="market-factor-kv"><div><dt>{t("Family")}</dt><dd>{detail.family ?? "unavailable"}</dd></div><div><dt>{t("Mechanism")}</dt><dd>{detail.mechanism ?? "unavailable"}</dd></div><div><dt>{t("Hypothesis")}</dt><dd>{detail.hypothesis ?? "unavailable"}</dd></div><div><dt>{t("Origin")}</dt><dd>{detail.origin ?? "unavailable"}</dd></div></dl><pre className="json-view" data-testid="factor-lifecycle">{json(detail.lifecycle)}</pre></article>
        <article className="factor-intelligence-card"><h3>{t("Provenance / Agent decisions")}</h3><pre className="json-view" data-testid="factor-provenance">{json(detail.provenance)}</pre>{detail.agent_decision_history.length ? <pre className="json-view">{json(detail.agent_decision_history)}</pre> : <div className="market-factor-unavailable">Agent decision history unavailable for this exact factor identity.</div>}</article>
      </div>
      <div className="market-factor-grid"><article className="factor-intelligence-card"><h3>{t("Global development metrics")}</h3>{detail.global_metrics ? <pre className="json-view" data-testid="factor-global-metrics">{json(detail.global_metrics)}</pre> : <div className="market-factor-unavailable">unavailable · {detail.evaluation_selection_reason}</div>}</article><article className="factor-intelligence-card"><h3>{t("MarketState-conditioned metrics")}</h3>{detail.state_metrics ? <pre className="json-view" data-testid="factor-state-metrics">{json(detail.state_metrics)}</pre> : <div className="market-factor-unavailable">unavailable</div>}</article></div>
      <div className="market-factor-grid"><article className="factor-intelligence-card"><h3>{t("Cost-sensitive economics")}</h3>{detail.cost_sensitive_economics ? <pre className="json-view">{json(detail.cost_sensitive_economics)}</pre> : <div className="market-factor-unavailable">unavailable</div>}</article><article className="factor-intelligence-card"><h3>{t("Similarity / novelty")}</h3>{detail.similarity ? <pre className="json-view">{json(detail.similarity)}</pre> : <div className="market-factor-unavailable">similarity unavailable</div>}<p>novelty: {detail.novelty_status}</p></article></div>
      <article className="factor-intelligence-card"><h3>{t("Persisted allocator weight history")}</h3>{detail.allocator_weights.length ? <div className="factor-weight-list" data-testid="factor-weights">{detail.allocator_weights.map((row, index) => <div key={`${String(row.as_of)}-${String(row.allocator)}-${index}`}><time>{String(row.as_of)}</time><strong>{String(row.allocator)}</strong><code>weight={String(row.weight)}</code><span>{String(row.state_link_status)}</span></div>)}</div> : <div className="market-factor-unavailable">unavailable · no persisted allocator snapshot binds this factor.</div>}</article>
      <article className="factor-intelligence-card"><h3>{t("Canonical linked research")}</h3><div className="market-linked-actions">{detail.linked_market_state_model_ids.map((modelId) => <Link key={modelId} to={`/market?market_model=${encodeURIComponent(modelId)}${search ? `&${search.slice(1)}` : ""}`} onClick={() => select({ market_state_model_id: modelId }, "market_state_selected")}><Link2 size={12} /> MarketState:{modelId}</Link>)}{detail.linked_experiments.map((experiment) => <Link key={String(experiment.identity)} to={`/experiments?experiment=${encodeURIComponent(String(experiment.identity))}${search ? `&${search.slice(1)}` : ""}`}><Link2 size={12} /> experiment:{String(experiment.identity)}</Link>)}<Link to={`/research-graph${search}`}><GitBranch size={12} /> {t("Research Graph")}</Link><Link to={`/strategy${search}`}><Link2 size={12} /> {t("Strategy / terminal")}</Link>{detail.agent_decision_history.map((decision) => decision.run_id ? <Link key={String(decision.node_id)} to={`/agent?run=${encodeURIComponent(String(decision.run_id))}${search ? `&${search.slice(1)}` : ""}`}>Agent run:{String(decision.run_id)}</Link> : null)}</div><pre className="json-view">{json(detail.evidence_identities)}</pre></article>
    </div> : null}
