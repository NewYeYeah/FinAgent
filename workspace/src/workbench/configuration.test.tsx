...
const registry = {
  ...
  descriptors: [{
    ...
    read_only: true,
    fields: [
      { field_path: "root", label: "root", value_type: "string", domain: "runtime", mutation_policy: "restart_or_new_run", required: true, secret_redacted: false, description: "" },
      { field_path: "universe_top_n", label: "universe top n", value_type: "integer", domain: "research_protocol", mutation_policy: "new_identity_required", required: true, secret_redacted: false, description: "" },
  ...
  snapshots: [
    {
      ...
      values: { root: "D:/Data/A-Share", universe_top_n: 150, secrets_file: "<secret-file-reference>" },
      domains: { root: "runtime", universe_top_n: "research_protocol", secrets_file: "secret_reference" },
      mutation_policies: { root: "restart_or_new_run", universe_top_n: "new_identity_required", secrets_file: "host_secret_binding_only" },
    ...
    },
    {
      ...
      values: { root: "E:/Data/A-Share", universe_top_n: 180, secrets_file: "<secret-file-reference>" },
      domains: { root: "runtime", universe_top_n: "research_protocol", secrets_file: "secret_reference" },
      mutation_policies: { root: "restart_or_new_run", universe_top_n: "new_identity_required", secrets_file: "host_secret_binding_only" },
      redacted_fields: ["secrets_file"],
      read_only: true,
...
const diff = {
  ...
  read_only: true,
  changes: [
    { field_path: "root", before: "D:/Data/A-Share", after: "E:/Data/A-Share", domain: "runtime", mutation_policy: "restart_or_new_run", requires_new_identity: false },
    { field_path: "universe_top_n", before: 150, after: 180, domain: "research_protocol", mutation_policy: "new_identity_required", requires_new_identity: true },
  ],
};
...
describe("V3-2B Configuration and Command catalogs", () => {
  beforeEach(() => {
  ...
  it("renders read-only config domains, protected references and identity semantics", async () => {
    ...
    expect(await screen.findByText(/new governed identity is required/i)).toBeInTheDocument();
    expect(screen.getByText(/150 → 180/)).toBeInTheDocument();
  });

  it("navigates to the command catalog while keeping execution unavailable", async () => {
    render(<App />);
    await screen.findByText("Configuration Registry");
    const commandLink = screen.getByRole("link", { name: "Command Catalog" });
    expect(commandLink).toHaveAttribute("href", "/widgets?program=program-a&surface=commands");
    ...
    expect(screen.getByRole("button", { name: /Run A2.6 robust research execution disabled/i })).toBeDisabled();
    expect(screen.getByText(/production_reserve/)).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /promote|reserve|broker|order/i })).not.toBeInTheDocument();
  });
});
