...
const seriesId = "factor-series-e2e-v43";
const programId = "program-e2e-v43";
const factorA = "factor-a-e2e-v43";
const factorB = "factor-b-e2e-v43";

const item = {
  series_id: seriesId,
  program_result_id: "program-result-e2e-v43",
  program_id: programId,
  data_version: "data-e2e-v43",
...
const detail = {
  ...
  manifest: {
    ...
    program_spec_id: "program-spec-e2e-v43",
    walk_forward_report_id: "walk-e2e-v43",
    gate_report_id: "gate-e2e-v43",
    selection_id: "selection-e2e-v43",
    plan_id: "plan-e2e-v43",
    candidate_selection_id: "candidate-e2e-v43",
    universe_policy_version: "universe-e2e-v43",
    quantiles: 3,
    ...
    rolling_window: 20,
    quant_config_digest: "quant-e2e-v43",
    rows_digest: "rows-e2e-v43",
    source_report_content_digest: "source-e2e-v43",
    source_report_file: "a2p6.json",
    source_report_sha256: "a".repeat(64),
    data_file: "factors.parquet",
...
const candidate = (digest: string, name: string, selected: boolean) => ({
...
const provenance = {
  ...
  agent_chronology_available: false,
  chronology_note: "A2.6 does not freeze Agent generation timestamps or rounds; V4-3 does not infer them.",
  series_id: seriesId,
  items: [
...
function rowsFor(url: string) {
  ...
  const rows = ["2024-01-02", "2024-01-03"].map((session, index) => ({
    sequence: index,
...
test("V4-3 renders verified factor analytics and preserves context without Control authority", async ({ page }) => {
  await page.route("http://127.0.0.1:8766/api/v3/control/**", (route) => route.abort("connectionrefused"));
  await page.route("**/api/v4/factor-series", (route) => route.fulfill({ json: catalog }));
  await page.route(`**/api/v4/factor-series/${seriesId}/dimensions`, (route) => route.fulfill({ json: dimensions }));
  await page.route(`**/api/v4/factor-series/${seriesId}/summary*`, (route) => route.fulfill({ json: summary }));
  await page.route(`**/api/v4/factor-series/${seriesId}/correlations`, (route) => route.fulfill({ json: correlation }));
  await page.route(`**/api/v4/factor-series/${seriesId}/heatmap*`, (route) => route.fulfill({ json: heatmap }));
  await page.route(`**/api/v4/factor-series/${seriesId}/provenance`, (route) => route.fulfill({ json: provenance }));
  await page.route(`**/api/v4/factor-series/${seriesId}/rows?*`, (route) => route.fulfill({ json: rowsFor(route.request().url()) }));
  await page.route(`**/api/v4/factor-series/${seriesId}`, (route) => route.fulfill({ json: detail }));

  await page.goto(`/factors/${seriesId}?program=${programId}&factor=${factorA}`);

  await expect(page.getByRole("heading", { name: "Alpha A" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Factors" })).toBeVisible();
  await expect(page.getByText("IC & rolling IC")).toBeVisible();
  await expect(page.getByText("IC decay")).toBeVisible();
  await expect(page.getByText("Quantile & long-short NAV")).toBeVisible();
  await expect(page.getByText("Fold / year RankIC heatmap")).toBeVisible();
  await expect(page.getByText("HAC / block-bootstrap inference forest")).toBeVisible();
  await expect(page.getByText("Holm / BH multiplicity matrix")).toBeVisible();
  await expect(page.getByText("Factor correlation cluster")).toBeVisible();
  await expect(page.getByText("Candidate provenance")).toBeVisible();
  await expect(page.getByText(/AGENT CHRONOLOGY NOT PERSISTED/i)).toBeVisible();
  await expect(page.getByText(/does not infer/i)).toBeVisible();
  ...
  await page.reload();
  await expect(page.getByText("IC & rolling IC")).toBeVisible();
  await expect(context).toContainText("wf-2");
  await expect(context).toContainText("2024-01-03..2024-02-02");
});
