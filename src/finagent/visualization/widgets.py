...
def default_widget_specs() -> tuple[FinWidgetSpec, ...]:
    ...
    values = (
        ...
        FinWidgetSpec(
            widget_id="a5.reserve.lifecycle",
            version="v1",
            surface=WidgetSurface.RESEARCH,
            question="Was the one-shot reserve consumed exactly once and can its terminal result be replay-audited?",
            evidence_types=(
                "ashare_reserve_eligibility_seal",
                "ashare_reserve_consumption_claim",
                "ashare_reserve_terminal_evidence",
                "ashare_reserve_consumption_audit",
            ),
            data_endpoint="/api/v2/reserves/{reserve_id}",
            data_schema="finagent.workspace.reserve-lifecycle.v1",
            renderer=WidgetRenderer.DAG,
            parameters=_parameters("reserve_id"),
            link_keys=("reserve_id",),
            lineage_refs=("reserve_id",),
        ),
        FinWidgetSpec(
            widget_id="a5.reserve.terminal",
            version="v1",
            surface=WidgetSurface.RESEARCH,
            question="What was the immutable terminal reserve outcome and did the durable evidence pass integrity checks?",
            evidence_types=("ashare_reserve_terminal_evidence", "ashare_reserve_execution_ledger"),
            data_endpoint="/api/v2/reserves/{reserve_id}",
            data_schema="finagent.workspace.reserve-lifecycle.v1",
            renderer=WidgetRenderer.METRIC_GROUP,
            parameters=_parameters("reserve_id"),
