...
from .historical_command_catalog import default_historical_command_catalog
from .workbench_control_api import (
    ControlRunRequest,
    _normalize_context,
    _safe_identifier,
)
...
HISTORICAL_CONTROL_API_VERSION = "finagent-workbench-control-api-ac1"


class HistoricalControlCommandRunner:
    """A-C1 runner for the reviewed historical L0/L1 application services only."""
    ...
    def __init__(
        ...
        *,
        ...
        self._catalog = default_historical_command_catalog()
        self._services = historical_application_service_registry(config_registry)
        ...
        self._executor = ThreadPoolExecutor(
            ...
            thread_name_prefix="finagent-historical-control",
        )
    ...
    def _execute(self, command_run_id: str) -> None:
        try:
            ...
            if spec.gateway_readiness != "application_service_ready":
                raise RuntimeError("historical command readiness changed before execution")
            if (
                HISTORICAL_APPLICATION_SERVICE_BINDINGS.get(spec.command_id)
                != spec.binding_ref
            ):
                raise RuntimeError("historical catalog/application binding drift detected")
            ...
            if record.intent.config_snapshot_id is not None:
                ...
                if spec.command_id != "config.validate" and snapshot.redacted_fields:
                    raise RuntimeError(
                        "Historical Control Plane cannot resolve host-bound secret references"
                    )
...
def _validate_historical_request(
    ...
    *,
    ...
    services = historical_application_service_registry(config_registry)
    ...
    if spec.level not in {"L0", "L1"}:
        return False, "Historical Control Plane permits only L0/L1 commands", parameters, snapshot_id, context
    if spec.gateway_readiness != "application_service_ready":
    ...
    if spec.config_descriptor_ids:
        ...
        if snapshot.descriptor_id not in spec.config_descriptor_ids:
            return False, "config snapshot descriptor is not permitted for this command", parameters, snapshot_id, context
    ...
    if command_id == "review.export_bundle":
        ...
        if not validation_id:
            return False, "review.export_bundle requires portfolio_validation_id context", parameters, snapshot_id, context
        try:
...
def _historical_catalog_projection(config_registry: ConfigRegistry) -> dict[str, object]:
    catalog = default_historical_command_catalog()
    services = historical_application_service_registry(config_registry)
    ...
    for spec in catalog.specs:
        executable = (
            ...
            and HISTORICAL_APPLICATION_SERVICE_BINDINGS.get(spec.command_id)
            == spec.binding_ref
        )
    ...
    return {
        "schema_version": "finagent.workbench.control-command-catalog.ac1",
...
def create_historical_control_app(
    *,
    ...
    runner = HistoricalControlCommandRunner(
        store=store,
    ...
    if ready != runner.service_ids:
        raise RuntimeError(
            "historical application-service-ready catalog does not match registered services"
        )
    ...
    app = FastAPI(
        title="FinAgent Historical Workbench Control Plane",
        version=HISTORICAL_CONTROL_API_VERSION,
        lifespan=lifespan,
    )
    app.add_middleware(
        CORSMiddleware,
    ...
    app.state.historical_operations = True
    ...
    def get_control_status() -> dict[str, object]:
        return {
            "schema_version": "finagent.workbench.control-status.ac1",
            "version": HISTORICAL_CONTROL_API_VERSION,
            "control_plane_enabled": True,
            "historical_operations": True,
            "local_only": True,
    ...
    def get_control_commands() -> dict[str, object]:
        return _historical_catalog_projection(config_registry)
    ...
    def create_control_run(request: ControlRunRequest):
        ...
        command_id = request.command_id.strip()
        try:
            accepted, reason, parameters, snapshot_id, context = _validate_historical_request(
                request,
                config_registry=config_registry,
            )
        except ValueError as exc:
...
def create_app_from_environment() -> FastAPI:
    ...
    return create_historical_control_app(
        config_paths=config_paths,
        report_paths=report_paths,
        store_path=os.environ.get(
            "FINAGENT_CONTROL_STORE", ".finagent/workbench/commands.sqlite"
        ),
        export_dir=os.environ.get(
            "FINAGENT_CONTROL_EXPORT_DIR", ".finagent/workbench/exports"
        ),
        requested_by=os.environ.get(
            "FINAGENT_CONTROL_ACTOR", "local-workbench-user"
        ),
