...
_SAFE_SECRET_REFERENCE_KEYS = {"secret_id", "secrets_file", "api_key_env", "token_env"}
_RUNTIME_KEYS = {
    "root",
    "frozen_manifest",
    "supplement_root",
...
_GUARDRAIL_PREFIXES = ("policy_", "limit_", "max_participation", "kill_switch")
_SUPPORTED_SECTIONS: dict[str, tuple[str, ConfigDomain]] = {
    "local_ashare_robust_research": ("A2.6 robust A-share research", "research_protocol"),
    "local_ashare_factor_research": ("A2/A2.5 A-share factor research", "research_protocol"),
    "local_ashare_research_smoke": ("A-share research smoke", "research_protocol"),
    "ashare_portfolio_validation": ("A4 portfolio validation", "research_protocol"),
    "ashare_execution_smoke": ("A3 execution smoke", "execution_protocol"),
...
def _flatten(value: Mapping[str, object], prefix: str = "") -> dict[str, JsonValue]:
    ...
    for raw_key, raw_value in sorted(value.items(), key=lambda item: str(item[0])):
        ...
        if isinstance(raw_value, Mapping):
            ...
            output[path] = _json_value(raw_value)
    return output
...
def _is_forbidden_secret_key(field_path: str) -> bool:
    ...
    if leaf in _SAFE_SECRET_REFERENCE_KEYS:
        return False
    return leaf in _FORBIDDEN_SECRET_KEYS or leaf.endswith("_password") or leaf.endswith("_token")


def _domain_for(section: str, field_path: str, default: ConfigDomain) -> ConfigDomain:
    leaf = _leaf_name(field_path)
    ...
    if section in {"ashare_execution_smoke", "ashare_portfolio_validation"} and (
        leaf.startswith(_EXECUTION_PREFIXES) or leaf in {"cash_fallback_on_model_error"}
    ):
        return "execution_protocol"
    if section == "ashare_portfolio_validation" and leaf.startswith(_GUARDRAIL_PREFIXES):
...
def _sanitize_value(field_path: str, value: JsonValue) -> tuple[JsonValue, bool]:
    leaf = _leaf_name(field_path)
    if leaf == "secrets_file":
        return _SECRET_FILE_REFERENCE, True
...
class ConfigRegistry:
    ...
    Secret files are never parsed. Direct credential-looking fields are redacted even when
    they appear in an otherwise public file. The registry is presentation metadata only and
    never mutates application configuration.
    """
    ...
    def __init__(self, roots: Sequence[str | Path]) -> None:
        ...
        self._descriptors = {item.descriptor_id: item for item in self._projection.descriptors}
        self._snapshots = {item.snapshot_id: item for item in self._projection.snapshots}
    ...
    def snapshots(self, descriptor_id: str | None = None) -> tuple[ConfigSnapshot, ...]:
        if descriptor_id is None:
            return self._projection.snapshots
        self.descriptor(descriptor_id)
        return tuple(
            item for item in self._projection.snapshots if item.descriptor_id == descriptor_id
        )
    ...
    def diff(self, left_snapshot_id: str, right_snapshot_id: str) -> ConfigDiff:
        ...
        field_paths = sorted(set(left.values) | set(right.values))
        ...
        for field_path in field_paths:
            ...
            policy = right.mutation_policies.get(field_path) or left.mutation_policies[field_path]
            changes.append(
                ConfigDiffItem(
                    ...
                    in {"research_protocol", "execution_protocol", "operational_guardrail"},
                )
            )
    ...
    def _build(self) -> ConfigRegistryProjection:
        ...
        fields_by_descriptor: dict[str, dict[str, ConfigFieldSpec]] = {}
        ...
        for root_index, root, path in self._candidate_files():
            if _contains_secret_filename(path):
                warnings.append(
                    f"secret-like config excluded without parsing: config-root-{root_index}/{path.name}"
                )
                continue
            try:
                ...
                warnings.append(f"config skipped: {path.name}: {type(exc).__name__}")
                continue
            ...
            for section, (title, default_domain) in _SUPPORTED_SECTIONS.items():
                ...
                for field_path, raw_value in flattened.items():
                    value, redacted = _sanitize_value(field_path, raw_value)
                ...
                snapshot_ids_by_descriptor.setdefault(descriptor_id, []).append(snapshot_id)
        ...
        for descriptor_id in sorted(fields_by_descriptor):
            ...
            descriptors.append(
                ConfigDescriptor(
                    ...
                    fields=tuple(
                        fields_by_descriptor[descriptor_id][key]
                        for key in sorted(fields_by_descriptor[descriptor_id])
                    ),
                    snapshot_ids=tuple(sorted(snapshot_ids_by_descriptor[descriptor_id])),
                )
            )
        snapshots.sort(key=lambda item: (item.descriptor_id, item.source_uri, item.snapshot_id))
...
class CommandSpec:
    ...
    def __post_init__(self) -> None:
        if self.level not in {"L0", "L1"}:
            raise ValueError("V3-2B generic command catalog may contain only L0/L1 commands")
...
_DEFAULT_COMMAND_SPECS = (
    CommandSpec(
        ...
        description="Validate a public configuration snapshot before any governed execution is considered.",
        level="L0",
        config_descriptor_ids=tuple(sorted(_SUPPORTED_SECTIONS)),
        binding_kind="registry",
        binding_ref="finagent.visualization.workbench_control_catalog.ConfigRegistry",
    ...
    CommandSpec(
        ...
        description="Run deterministic local-data certification without research or trading authority.",
        level="L0",
    ...
    CommandSpec(
        ...
        description="Future governed L1 entry for bounded A2/A2.5 development research only.",
        level="L1",
    ...
    CommandSpec(
        ...
        description="Future governed L1 entry for preregistered robust ResearchProgram execution.",
        level="L1",
    ...
    CommandSpec(
        ...
        description="Future L0 command for deterministic read-only human-review bundle export.",
        level="L0",
        config_descriptor_ids=(),
        binding_kind="workspace_projection",
        binding_ref="finagent.visualization.workspace_v2.WorkspaceV2Projection",
        gateway_readiness="adapter_required",
...
class CommandCatalog:
    def __init__(self, specs: Sequence[CommandSpec] = _DEFAULT_COMMAND_SPECS) -> None:
