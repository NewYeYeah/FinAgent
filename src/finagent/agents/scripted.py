...
from .domain import AgentDecision, AgentDecisionStatus, AgentRunContext, AgentTask, ToolCallRequest, ToolCallResult, ToolCallStatus
...
class ScriptedResearchAgent:
    """Deterministic Phase 3B runtime using only the governed ToolRegistry surface."""

    def __init__(self, *, plan: ResearchPlan, templates: ExperimentTemplateRegistry, plan_store: SQLiteAgentPlanStore, clock: Callable[[], datetime] | None = None) -> None:
        self.plan = plan
    ...
    def run(self, task: AgentTask, tools: ToolRegistry, context: AgentRunContext) -> AgentDecision:
        ...
        def invoke(tool_name: str, arguments: Mapping[str, object]) -> ToolCallResult:
            ...
            result = tools.invoke(ToolCallRequest(call_id, tool_name, arguments, self.clock()), context)
        ...
        def terminal(result: ToolCallResult, action: str) -> AgentDecision | None:
            if result.status is ToolCallStatus.SUCCEEDED:
                return None
            status = AgentDecisionStatus.BLOCKED if result.status in {ToolCallStatus.DENIED, ToolCallStatus.REQUIRES_APPROVAL} else AgentDecisionStatus.FAILED
            return AgentDecision(
                ...
                self.clock(),
                tuple(call_ids),
                {"plan_fingerprint": fingerprint, "family_id": self.plan.family_id, "waiting_for_approval": str(result.status is ToolCallStatus.REQUIRES_APPROVAL).lower()},
            )

        state = invoke("list_experiment_families", {})
        if (decision := terminal(state, "inspect research state")) is not None:
            return decision
        if any(isinstance(item, Mapping) and item.get("family_id") == self.plan.family_id for item in state.output.get("families", ())):
            return AgentDecision(context.run_id, AgentDecisionStatus.BLOCKED, f"experiment family {self.plan.family_id!r} already exists", self.clock(), tuple(call_ids), {"plan_fingerprint": fingerprint})
        if not self.plan.budget.allow_new_family:
            return AgentDecision(context.run_id, AgentDecisionStatus.BLOCKED, "research budget forbids creating a new family", self.clock(), tuple(call_ids), {"plan_fingerprint": fingerprint})

        created = invoke("create_experiment_family", {
            ...
            "metadata": {"plan_id": self.plan.plan_id, "planner_version": self.plan.planner_version},
        })
        ...
        for variant in self.plan.variants:
            result = invoke("register_experiment", template.materialize(family_id=self.plan.family_id, variant=variant))
        ...
        for variant in self.plan.variants:
            ...
            if result.status is ToolCallStatus.FAILED:
                ...
                if failed_experiments > self.plan.budget.max_failed_experiments:
                    return AgentDecision(context.run_id, AgentDecisionStatus.FAILED, f"failed experiment budget exceeded after {variant.experiment_id}: {result.error}", self.clock(), tuple(call_ids), {"plan_fingerprint": fingerprint, "family_id": self.plan.family_id, "failed_experiments": str(failed_experiments)})
                continue
        ...
        primary = invoke("compare_experiment_results", {"experiment_ids": experiment_ids, "metric": self.plan.primary_metric})
        if (decision := terminal(primary, "compare primary metric")) is not None:
            return decision
        tie = None
        if self.plan.tie_break_metric:
            tie = invoke("compare_experiment_results", {"experiment_ids": experiment_ids, "metric": self.plan.tie_break_metric})
            if (decision := terminal(tie, "compare tie-break metric")) is not None:
                return decision

        selection = self._select_winner(primary.output, tie.output if tie is not None else None)
        self.plan_store.append_event(context.run_id, "selection_sealed", {
        ...
        if (decision := terminal(frozen, "freeze experiment family")) is not None:
            return decision
        validation = invoke("validate_experiment_family", {"family_id": self.plan.family_id, "selected_experiment_id": selection.experiment_id})
        ...
        if validation_passed and self.plan.promotion_intent is not None and self.plan.budget.allow_promotion_request:
            intent = self.plan.promotion_intent
            promotion = invoke("request_model_promotion", {"model_id": intent.model_id, "to_stage": intent.to_stage, "reason": intent.reason})
            if promotion.status is ToolCallStatus.REQUIRES_APPROVAL:
                metadata["waiting_for_approval"] = "true"
                return AgentDecision(context.run_id, AgentDecisionStatus.BLOCKED, "research completed; promotion request awaits human approval", self.clock(), tuple(call_ids), metadata)
            if (decision := terminal(promotion, "request model promotion")) is not None:
                return decision

        summary = "scripted research completed and family validation passed" if validation_passed else "scripted research completed but family validation did not pass"
        return AgentDecision(context.run_id, AgentDecisionStatus.COMPLETED, summary, self.clock(), tuple(call_ids), metadata)

    @staticmethod
    def _select_winner(primary_output: Mapping[str, object], tie_break_output: Mapping[str, object] | None) -> WinnerSelection:
        raw = primary_output.get("comparisons")
        if not isinstance(raw, (list, tuple)):
            raise ValueError("invalid primary comparison payload")
        primary: dict[str, float] = {}
        for item in raw:
            if isinstance(item, Mapping) and item.get("value") is not None and item.get("passed") is not False:
        ...
        winner = min(primary, key=lambda exp: (-primary[exp], tie.get(exp, float("inf")), exp))
