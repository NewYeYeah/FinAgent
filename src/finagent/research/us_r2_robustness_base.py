...
class USR2RobustnessSlice:
    ...
    def __post_init__(self) -> None:
        if self.kind not in {"frequency", "decay"}:
            raise ValueError("US-R2 robustness slice kind must be frequency or decay")
        allowed = {
        ...
        if (self.kind, self.signal_interval, self.label_horizon_trading_minutes) not in allowed:
            raise ValueError("US-R2 robustness slice differs from frozen R1 semantics")
        expected = (
            f"{self.kind}_{self.signal_interval.value}_"
            f"{self.label_horizon_trading_minutes}m"
        )
...
class USR2RobustnessMaterializationPolicy:
    ...
    source_execution_strategy: str = "one_materialized_sessionized_1m_cte_per_year"
    ...
    def __post_init__(self) -> None:
        if self.research_protocol_id != canonical_us_r1_research_protocol().protocol_id:
            raise ValueError("US-R2 robustness policy must inherit the frozen R1 research protocol")
        if self.formation_policy_id != canonical_us_r1_feature_formation_policy().policy_id:
            raise ValueError("US-R2 robustness policy must inherit the frozen R1 formation policy")
        if self.pooled_inference_report_id != FROZEN_POOLED_INFERENCE_REPORT_ID:
            raise ValueError("US-R2 robustness policy must bind the reviewed pooled inference")
        if self.slices != canonical_us_r2_robustness_slices():
            raise ValueError("US-R2 robustness slices must remain the exact frozen R1 set")
        if self.source_execution_strategy != "one_materialized_sessionized_1m_cte_per_year":
            raise ValueError("US-R2 robustness source sharing policy changed")
    ...
    def to_dict(self, *, include_id: bool = True) -> dict[str, object]:
        ...
        payload: dict[str, object] = {
            ...
            "resampling_spec_ids": resampling_specs,
            "label_spec_ids": label_specs,
            "source_execution_strategy": self.source_execution_strategy,
            "raw_1m_required": True,
            "derive_robustness_from_primary_15m60m_cache": False,
            "candidate_performance_filter_applied": False,
...
def _slice_select(
    ...
    *,
    ...
    interval_literal = _sql_string(item.signal_interval.value)
    slice_literal = _sql_string(item.slice_id)
    kind_literal = _sql_string(item.kind)
    horizon = item.label_horizon_trading_minutes
    return f"""
        SELECT
            {slice_literal} AS slice_id,
            {kind_literal} AS slice_kind,
            {interval_literal} AS signal_interval,
            {horizon}::BIGINT AS label_horizon_trading_minutes,
            b.research_asset_id,
            b.session_date,
            b.session_id,
...
def build_us_r2_annual_robustness_base_plan(
    ...
    *,
    ...
    data_version = _canonical_hash(
        {
            ...
            "source_execution_strategy": policy.source_execution_strategy,
        },
    ...
    for interval in (BarInterval.MINUTE_5, BarInterval.MINUTE_15, BarInterval.MINUTE_30):
        ...
        bar_ctes.append(cte)
    label_ctes = [_label_cte(horizon) for horizon in (30, 60, 120)]
    slice_selects = [
        _slice_select(
            item,
            bars_relation=bar_relations[item.signal_interval],
            source_data_literal=source_data_literal,
    ...
    sql = f"""
        ...
        SELECT {', '.join(USR2AnnualRobustnessBasePlan.output_columns)}
        FROM slices
