...
def formal_agent_request_id(run_spec_id: str, *, slot_index: int, attempt_index: int) -> str:
    if slot_index < 1:
...
def _parse_usage(document: Mapping[str, object]) -> CandidateGenerationUsage:
    usage = CandidateGenerationUsage(
        llm_calls=_int(document.get("llm_calls"), "usage.llm_calls"),
        input_tokens=_int(document.get("input_tokens"), "usage.input_tokens"),
        output_tokens=_int(document.get("output_tokens"), "usage.output_tokens"),
        latency_ms=_float(document.get("latency_ms"), "usage.latency_ms"),
        cost_usd=_float(document.get("cost_usd"), "usage.cost_usd"),
    )
...
def _parse_proposal(document: Mapping[str, object]) -> StructuredCandidateProposal:
    proposal = StructuredCandidateProposal(
        ...
        window_bars=_int(document.get("window_bars"), "proposal.window_bars"),
        hypothesis_summary=_text(document.get("hypothesis_summary"), "proposal.hypothesis_summary"),
        generated_at=datetime.fromisoformat(_text(document.get("generated_at"), "proposal.generated_at")),
        usage=_parse_usage(_mapping(document.get("usage"), "proposal.usage")),
        parent_candidate_id=_optional_text(document.get("parent_candidate_id"), "proposal.parent_candidate_id"),
    )
...
class USAgentValueFormalAgentAttemptEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.request_id != formal_agent_request_id(
            self.run_spec_id,
        ...
        if self.status is not CandidateValidationStatus.INVALID and self.candidate_id is None:
            raise ValueError("non-INVALID FORMAL attempt requires candidate_id")

    @property
    def attempt_id(self) -> str:
        return _hash(self.to_dict(include_id=False), "us-agent-value-formal-agent-attempt")
...
class USAgentValueFormalAgentSlotEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        for attempt in (self.initial, self.repair):
            if attempt is None:
                continue
            if (
                attempt.execution_plan_id != self.execution_plan_id
                or attempt.launch_bundle_id != self.launch_bundle_id
                or attempt.runtime_policy_id != self.runtime_policy_id
                or attempt.run_spec_id != self.run_spec_id
                or attempt.run_ordinal != self.run_ordinal
                or attempt.slot_index != self.slot_index
            ):
                raise ValueError("FORMAL slot attempt identity mismatch")
        ...
        if self.initial.status is CandidateValidationStatus.VALID_UNIQUE and self.repair is not None:
            raise ValueError("valid initial proposal cannot receive FORMAL repair")
        if self.initial.status is not CandidateValidationStatus.VALID_UNIQUE and self.repair is None:
            raise ValueError("non-valid FORMAL initial proposal must consume its single repair")
    ...
    def slot_id(self) -> str:
        return _hash(self.to_dict(include_id=False), "us-agent-value-formal-agent-slot")
    ...
    def to_dict(self, *, include_id: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "repair_count": int(self.repair is not None),
            "replacement_count": 0,
            "status_authority": False,
            "stage_exit_authority": False,
            "agent_value_gate_authority": False,
...
def parse_us_a0_formal_agent_attempt(document: Mapping[str, object]) -> USAgentValueFormalAgentAttemptEvidence:
    attempt = USAgentValueFormalAgentAttemptEvidence(
        ...
        run_ordinal=_int(document.get("run_ordinal"), "attempt.run_ordinal"),
        slot_index=_int(document.get("slot_index"), "attempt.slot_index"),
        attempt_index=_int(document.get("attempt_index"), "attempt.attempt_index"),
...
def parse_us_a0_formal_agent_slot(document: Mapping[str, object]) -> USAgentValueFormalAgentSlotEvidence:
    ...
    slot = USAgentValueFormalAgentSlotEvidence(
        ...
        repair=(
            ...
            else parse_us_a0_formal_agent_attempt(_mapping(repair_document, "slot.repair"))
        ),
    )
...
def validate_us_a0_formal_slot_sequence(
    ...
    for expected_slot, slot in enumerate(slots, start=1):
        ...
        final_candidate: USAgentValueCandidateSpec | None = None
        for attempt in (slot.initial, slot.repair):
            if attempt is None:
                continue
            status, candidate, reason = _classify(attempt.proposal, accepted_ids)
            if attempt.status is not status:
                raise ValueError("FORMAL attempt stored status differs from canonical classification")
            if attempt.candidate_id != (None if candidate is None else candidate.candidate_id):
            ...
            if attempt is slot.final_attempt:
                final_candidate = candidate
        if slot.final_attempt.status is CandidateValidationStatus.VALID_UNIQUE:
            if final_candidate is None:
                raise ValueError("FORMAL valid final attempt lost canonical candidate")
            accepted_ids.add(final_candidate.candidate_id)
...
class USAgentValueFormalAgentRunProgress:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(self.completed_slot_ids)) != len(self.completed_slot_ids):
    ...
    def progress_id(self) -> str:
        return _hash(self.to_dict(include_id=False), "us-agent-value-formal-agent-run-progress")
